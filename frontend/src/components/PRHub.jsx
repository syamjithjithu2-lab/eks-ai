        ...
        <div className="p-10 sidebar-compact-padding h-full flex flex-col gap-10">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-4">
                    <div className="bg-white/80 border border-slate-300 py-2.5 px-6 rounded-2xl shadow-sm flex items-center gap-3">
                         <span className="text-[10px] font-black text-slate-800 uppercase tracking-widest">{filteredPrs.length} OPEN PRs</span>
            ...
            <div className="glass-card rounded-[3rem] flex-1 flex flex-col overflow-hidden bg-white/40">
                <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-white/20">
                    <div className="flex items-center gap-3 bg-white/80 border border-slate-200 rounded-2xl px-5 py-2.5 w-96 shadow-sm focus-within:ring-4 ring-indigo-50 transition-all">
                    ...
                    <div className="flex gap-2">
                        {['ALL', 'SCALING', 'FIX', 'SECURITY'].map(tag => (
                            <button key={tag} className="px-4 py-2 text-[10px] font-black rounded-xl border border-slate-200 text-slate-700 bg-white hover:bg-slate-50 transition-all uppercase tracking-widest">
                                {tag}
                            </button>
                        ))}
                    </div>
                ...
                <div className="flex-1 overflow-auto p-8 custom-scrollbar">
                    {filteredPrs.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-center opacity-40">
                            ...
                            <p className="text-sm font-medium text-slate-500 mt-2 max-w-[300px]">The AI fleet is currently analyzing the system. New optimizations will appear here.</p>
                        </div>
                    ) : (
                                        ...
                                        <div className="flex items-center gap-6">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-3 mb-2">
                                                    <span className="text-[10px] font-black px-3 py-1 bg-indigo-600 text-white rounded-lg uppercase tracking-widest shadow-lg shadow-indigo-100">
                                                        PR #{Math.floor(Math.random() * 900) + 100}
                                                    </span>
                                                    <span className="text-[10px] font-black text-slate-700 uppercase tracking-widest">{pr.cluster}</span>
                                                    <span className="text-[10px] font-bold text-slate-400 font-tech">• {pr.status}</span>
                                                </div>
