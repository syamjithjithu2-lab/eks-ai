...
const AgentsPage = memo(({ clusters = [], incidents = [], prs = [] }) => {
    ...
    const handleSend = () => {
        ...
        setTimeout(() => {
            ...
            setMessages(prev => [...prev, { role: 'ai', text: responseText, timestamp: new Date().toLocaleTimeString() }]);
            setIsTyping(false);
        }, 1200);
    };
    ...
    const generateAIResponse = (query) => {
        ...
        if (q.includes('how many pr') || q.includes('number of pr')) {
            ...
            return `There are currently ${count} open Pull Requests. ${count > 0 ? "Most focus on resource scaling and cost optimization." : "The system is currently fully optimized."}`;
        }
        ...
        const foundIncident = incidents.find(inc =>
            ...
            (q.includes(inc.cluster.toLowerCase()) || q.includes(inc.namespace.toLowerCase()))
        );
        if (foundIncident) {
            return `I've cross-referenced that pattern. It looks like a match for Incident #${foundIncident.id.substring(0,6)} in ${foundIncident.namespace}. Root Cause: ${foundIncident.rootCause}.`;
        }
        if (q.includes('hello') || q.includes('hi ')) {
            return "Hello! I'm ready to assist. Ask me about cluster health, open PRs, or paste an error for root-cause analysis.";
        }
        return "I've analyzed that request. Based on current telemetry, everything seems stable. Check the 'Incidents' tab for deeper log analysis if you suspect latent issues.";
    };
    ...
    return (
        <div className="p-4 md:p-8 h-full flex flex-col gap-6 md:gap-8">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl md:text-3xl font-black text-slate-800 tracking-tight">Agents</h1>
            ...
            {/* Agent cards — 1 col mobile, 3 col lg */}
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
                {agents.map((agent, i) => (
                    <div key={i} className="glass-card rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 group">
                        <div className="flex justify-between items-start mb-6 md:mb-8">
                            <div className="w-12 h-12 md:w-14 md:h-14 bg-indigo-50 rounded-2xl flex items-center justify-center border border-indigo-100 group-hover:scale-110 transition-transform">
                        ...
                        <h3 className="text-lg md:text-xl font-black text-slate-800 tracking-tight">{agent.name}</h3>
                        <p className="text-[0.625rem] font-bold text-slate-600 uppercase tracking-widest mt-1">Edge Compute Unit</p>
                        <div className="grid grid-cols-2 gap-3 md:gap-4 mt-6 md:mt-8">
                            <div className="bg-white/80 p-3 md:p-4 rounded-2xl border border-slate-200 shadow-sm">
                                <p className="text-[0.625rem] font-black text-slate-700 uppercase tracking-widest mb-1">Latency</p>
                                <p className="text-xl md:text-2xl font-black text-slate-800">{agent.latency}</p>
                            </div>
                            <div className="bg-white/80 p-3 md:p-4 rounded-2xl border border-slate-200 shadow-sm">
                                <p className="text-[0.625rem] font-black text-slate-700 uppercase tracking-widest mb-1">Uptime</p>
                                <p className="text-xl md:text-2xl font-black text-emerald-600">{agent.uptime}</p>
                            </div>
                        </div>
                    </div>
                ))}
            ...
            {/* Chat + Stats — stack on mobile, side-by-side on lg */}
            <div className="flex flex-col lg:flex-row gap-6 md:gap-8 min-h-[24rem] lg:min-h-[37.5rem]">
                {/* AI Chat Panel */}
                <div className="flex-1 glass-card rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-8 flex flex-col relative overflow-hidden bg-white/60">
                    ...
                    <div className="flex items-center gap-4 mb-6 md:mb-8">
                        <div className="w-10 h-10 md:w-12 md:h-12 bg-gradient-to-tr from-indigo-600 to-violet-500 rounded-xl md:rounded-2xl flex items-center justify-center shadow-lg shadow-indigo-100">
                            <MessageSquare className="text-white" size={20} />
                        </div>
                        <div>
                            <h3 className="text-lg md:text-xl font-black text-slate-800 tracking-tight">AI Cluster Assistant</h3>
                    ...
                    <div className="flex-1 space-y-4 md:space-y-6 overflow-auto pr-2 mb-4 md:mb-6 custom-scrollbar" ref={scrollRef}>
                        {messages.map((msg, i) => (
                            <div key={i} className={`flex gap-3 md:gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                                ...
                                <div className={`${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-white/80 text-slate-700 rounded-tl-none border border-slate-100 shadow-sm'} p-3 md:p-4 rounded-2xl max-w-[85%]`}>
                            ...
                            <div className="flex gap-4">
                                ...
                                <div className="bg-white/80 p-4 rounded-2xl rounded-tl-none border border-slate-100 shadow-sm">
                                    <div className="flex gap-1">
                                        {[0,1,2].map(d => <div key={d} className="w-1.5 h-1.5 bg-indigo-400 rounded-full animate-bounce" style={{animationDelay:`${d*75}ms`}} />)}
                                    </div>
                                </div>
                            </div>
                        )}
                    ...
                    <div className="relative">
                        <input type="text" value={inputValue}
                            ...
                            className="w-full bg-slate-50 border border-slate-200 rounded-[2rem] px-6 md:px-8 py-4 md:py-5 text-sm font-bold text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-4 ring-indigo-50 transition-all shadow-inner"
                        />
                        <button onClick={handleSend}
                            className="absolute right-2.5 top-2 w-9 h-9 md:w-10 md:h-10 bg-indigo-600 rounded-full flex items-center justify-center text-white shadow-lg shadow-indigo-100 hover:scale-110 transition-transform"
                        ><Send size={16} fill="currentColor" /></button>
                    </div>
                </div>

                {/* Side panel — hidden on mobile, shown on lg+ as column */}
                <div className="w-full lg:w-[clamp(16rem,20vw,22rem)] flex flex-row lg:flex-col gap-4 md:gap-6">
                    <div className="flex-1 lg:flex-none glass-card rounded-[2rem] md:rounded-[2.5rem] p-5 md:p-6 bg-emerald-50/50 border-emerald-100/50">
                    ...
                    <div className="flex-1 lg:flex-none glass-card rounded-[2rem] md:rounded-[2.5rem] p-5 md:p-6">
